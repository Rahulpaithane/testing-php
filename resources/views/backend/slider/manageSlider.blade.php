...
@section('title', 'Manage Banner/Slider')
@section('papgeTitle', 'Manage Banner/Slider')
 
<section> 
    <div class="row">
        <div class="col-md-12">
            <div class="edu_btn_wrapper sectionHolder padderBottom30 text-right">
                <button type="button" class="btn btn-primary" onClick="openBannerModal()" >
                    <i class="fa fa-plus" aria-hidden="true"></i>New Banner
                </button>
            </div>
...
@include('backend.slider.newSlider')
<script>
    $(function() {
        load_data("","",""); // first load
    });
    function load_data(initial_date='', final_date='', current_date='' ){
        var ajax_url = '{{route($shared['routePath'].".bannerList")}}';
