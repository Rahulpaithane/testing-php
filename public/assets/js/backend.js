...
$(".basic_datatable").DataTable({
	...
	$(document).on('click','.removeAccHeading',function(){
		...
		$(document).on('click','.batchDelete0101',function(){
		    var element = this;
                    ...
                    .then((willDelete) => {
                      if (willDelete) { 
        			    if($(this).attr('id')!=''){
                				...
                				method: "POST",
                				url: base_url+"ajaxcall/deleteData",
                				data: {'id':$(this).attr('id'),'table':'student_payment_history'},
                				success: function(resp){
                					// alert(resp);
                					var resp = $.parseJSON(resp);
	...
	$(document).on('click','.edit_course, .add_course, .add_blog,.edit_blog,.edit_currentAffairs', function(){
		...
		}else if($(this).hasClass('edit_currentAffairs')){
			// parentDiv.find('#blogModalLabel').html(ltr_add_blog);
			parentDiv.find('#c_crsImg').addClass('require').closest('.form-group').find('label sup').removeClass('hide');
			parentDiv.find('.fileNameShow').html('');
			parentDiv.find('form').trigger('reset');
			parentDiv.find('#modal_type').val('edit');
			parentDiv.find('.addEditBlog').attr('data-type','add');
			parentDiv.find('#blogTitle').val($(this).closest('tr').find('td:eq(3)').text());
			parentDiv.find('.fileNameShow').html($(this).attr('data-img'));
			parentDiv.find('#blog_id').val($(this).attr('data-id'));
			parentDiv.find('#c_crsImg').removeClass('require');
			parentDiv.find('#c_short_desc').val($(this).attr('data-short-des'));
			parentDiv.find('#source_url').val($(this).attr('data-sourceUrl'));
			parentDiv.find('#start_date').val($(this).attr('data-publish-date'));
			$(".summernote").summernote("code",$(this).attr('data-des'));
...
$(document).on('click','.MultiappointmentDate', function(){
	...
	$(document).on('click','.addCurrentAffairsButton', function(){
		var formdata = new FormData($('#currentAffairsForm')[0]);
		var type = $(this).attr('data-type');
		formdata.append('type',type)
		var valid_check = validate_form($(this).closest('form'));
		targetTableUrl = $('.server_datatable').attr('data-url');
		if(valid_check == 'valid'){
			...
			$.ajax({
				method: "POST",
				url: base_url+"ajaxcall/addCurrentAffairs",
				data: formdata,
				processData: false, 
				contentType: false,
				success: function(resp){
			...
			$.ajax({
				...
				success: function(resp){
					...
					if(resp['status'] == '1'){
						...
						toastr.success('Batch Already Purchased');
					}else{
						toastr.error(ltr_something_msg);
					}
				},
	...
	$(document).on('click','.add_Newquestion',function(){
		...
		// var value = summernote.instances['question'].getData();
		// var option1 = summernote.instances['option1'].getData();
		// var option2 = summernote.instances['option2'].getData();
		// var option3 = summernote.instances['option3'].getData();
		// var option4 = summernote.instances['option4'].getData();
		// var option5 = summernote.instances['option5'].getData();
	...
	var callAjax = function(vid){
	   // alert(vid);
        $.ajax({
              type: "POST",
              url: base_url+"ajaxcall/live_video_comment",
              data: 'id=' + vid,
              success: function(data) {
                ...
                //   alert(result);
                  ...
                  setInterval(callAjax(vid),30000);
	...
	$(document).on('click','.callComment',function(){
	    $(".callComment").attr("disabled", true);
	    $('.callComment').html('<i class="fa fa-spinner fa-spin"></i> Show Comment');
      var vid= parseVideo($(this).attr('data-t'));
      callAjax(vid);
      $('#commentData').scrollTop($('#commentData')[0].scrollHeight);
      
    });
	$(document).on('click','.viewVideo',function(){
		var src = parseVideo($(this).attr('data-url'),$(this).attr('data-type'));
		var vid= parseVideo($(this).attr('data-id'));
// 		console.log(src);
		var html = '<div class="col-lg-7 col-md-7 col-sm-7 col-12"><iframe width="100%"  height="100%" src="'+src+'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>'+
		           '<div class="col-lg-5 col-md-5 col-sm-5 col-12"><div class="chat-history"><ul id="commentData"><li><button class="callComment btn btn-success" data-t="'+vid+'"  >Show Comment</button></li></ul></div></div>';
...
// 		
	...
	$(document).on('click','.addNewLiveVideo',function(){
	    // alert('helllo');
		var validchk = validate_form($(this).closest('form'));
		if(validchk == 'valid'){
			console.log($('input[name=video_type]:checked').val());
			...
			if($("#id").val() ==''){
				var url ='https://www.youtube.com/embed/'+$.trim($('#video_url').val());
			} else {
			...
			$.ajax({
				method: "POST",
				url: base_url+"ajaxcall/add_live_video",
				data: {'title':$.trim($('.video_title').val()),'batch':$.trim($('.video_batch').val()),'subject':$.trim($('.filter_subject option:selected').text()),'topic':$.trim($('.filter_chapter option:selected').text()),'video_live_date':$.trim($('.chooseDate').val()),'video_live_time':$.trim($('.chooseTime').val()),'url':url,'video_type':$.trim($('input[name=video_type]').val()),'id':$.trim($('input[name=id]').val())},
				success: function(resp){
					var resp = $.parseJSON(resp);
					if(resp['status'] == '1'){
						toastr.success(resp['msg']);
						targetTableUrl = $('.server_datatable').attr('data-url');
						dataTableObj.ajax.url(base_url+targetTableUrl).load();
						$('.pxn_amin2 form').find('form').trigger('reset');
						var no_data = $('.eac_page_re').html();
                        if(no_data !=undefined){
                          setTimeout(function(){
                          		window.location.reload(true);
	...
	$(document).on('click','.addNewTeacher',function(){
		...
		//     }
			$('#input_feilds_teacher').find('form').trigger('reset');
			$('#input_feilds_teacher').find('.edu_sub_title').html('Edit Book');

			$('#book_id').val($(this).attr('data-id'));
			$('#name').val($(this).attr('data-name'));
			$('#book_type option[value="'+$(this).attr("data-type")+'"]').attr("selected", true).change();
			$('#author').val($(this).attr('data-author'));
			$('#publication').val($(this).attr('data-publication'));
			$('#class option[value="'+$(this).attr("data-class")+'"]').attr("selected", true).change();
			$('#bookAmountType option[value="'+$(this).attr("data-bookAmountType")+'"]').attr("selected", true).change();
			$('#stock').val($(this).attr('data-stock'));
			$('#price').val($(this).attr('data-price'));
			$('#discount_on_wholesale').val($(this).attr('data-discount_on_wholesale'));
			$('#wholesaleQty').val($(this).attr('data-wholesaleQty'));
			$('.fileNameShow').html($(this).attr('data-image'));
	...
	$(document).on('click','.edit_scholarshipBatch',function(){
		// if($(this).hasClass('addBookPop')){
			var data=$(this).attr('data-id');
			// alert(data);

			$('#input_feilds_teacher').find('form').trigger('reset');
			$('#input_feilds_teacher').find('.edu_sub_title').html('Edit Scholarship Batch');

			$('#batch_id').val($(this).attr('data-id'));
			$('#scholarshipName').val($(this).attr('scholarshipName'));
			$('#session_starting_year option[value="'+$(this).attr("session_starting_year")+'"]').attr("selected", true).change();
			$('#session_ending_year option[value="'+$(this).attr("session_ending_year")+'"]').attr("selected", true).change();
			$('#start_publish_date').val($(this).attr('start_publish_date'));
			$('#end_publish_date').val($(this).attr('end_publish_date'));
			// $('#class option[value="'+$(this).attr("data-class")+'"]').attr("selected", true).change();
			// $('#bookAmountType option[value="'+$(this).attr("data-bookAmountType")+'"]').attr("selected", true).change();
			$('#last_modification_date').val($(this).attr('last_modification_date'));
			$('#input_feilds_teacher #admit_decleare_date').val($(this).attr('admit_decleare_date'));
			$('#input_feilds_teacher #exam_decleare_date').val($(this).attr('exam_decleare_date'));
			$('#result_out_date').val($(this).attr('result_out_date'));
			$('#fees').val($(this).attr('fees'));
			// $('.fileNameShow').html($(this).attr('data-image'));
			// $("#attachment").removeAttr("required");
	...
	$(document).on('click','.generate_scholarship',function(){
		// if($(this).hasClass('addBookPop')){
			var data=$(this).attr('data-id');
			// alert(data);

			$('#input_feilds_teacher2').find('form').trigger('reset');
			// $('#input_feilds_teacher2').find('.edu_sub_title').html('Edit Scholarship Batch');

			$('#scholarshipBatchId2022').val($(this).attr('data-id'));
			$('#scholarshipName2013a').html($(this).attr('scholarshipName'));
			$('#totalStudent2013a').html($(this).attr('totalStudent'));
			$('#input_feilds_teacher2 #admit_decleare_date').val($(this).attr('admit_decleare_date'));
			$('#input_feilds_teacher2 #exam_decleare_date').val($(this).attr('exam_decleare_date'));
			$('#exam_start_time').val($(this).attr('exam_start_time'));
			$('#exam_end_time').val($(this).attr('exam_end_time'));
			$('#reportingTime').val($(this).attr('reportingTime'));
			$('#entryGateClosingTime').val($(this).attr('entryGateClosingTime'));
			$("#instructionToCandidates").summernote("code", $(this).attr('instructionToCandidates'));
			
			if($(this).attr('admitCardStatus') =='1'){
				$("#newGen").html(`
					<div class="edu_btn_wrapper">
							<input type="hidden" name="buttonType" value="re2019"> 
              <input type="submit" name="generate" value="Re-Generate" class="btn btn-primary" />

              <button type="button" class="btn btn-danger" onclick="CancelAdmitCard()" >Cancel Admit Card</button>
          </div>
					`);
				...
				$("#newGen").html(`
					<div class="edu_btn_wrapper">
					<input type="hidden" name="buttonType" value="new2022"> 
                <input type="submit" name="generate" value="Generate" class="btn btn-primary" />
            </div>
					`);
			}
			// var tableLength = $("#center_info_table tbody tr").length;

   //      var totalSit=0;
   //      for(x = 0; x < tableLength; x++) {
   //          var tr = $("#center_info_table tbody tr")[x];
   //          var count = $(tr).attr('id');
   //          // alert(count);
   //          var sit=$("#sit_"+count).val() =='' ? 0 : $("#sit_"+count).val();
   //          totalSit +=parseInt(sit);
   //    }
   //    $("#totalRowCapacity").val(totalSit);

			$.magnificPopup.open({
			items: {
				src: '#input_feilds_teacher2',
			},
			type: 'inline'
		});
	...
	$(document).on('click','.edit_examinationCenter',function(){
		// if($(this).hasClass('addBookPop')){
			var data=$(this).attr('data-id');
			// alert(data);

			$('#input_feilds_teacher').find('form').trigger('reset');
			$('#input_feilds_teacher').find('.edu_sub_title').html('Edit Examination Center');

			$('#center_id').val($(this).attr('data-id'));
			$('#centerName').val($(this).attr('centerName'));
			$('#centerCapacity').val($(this).attr('centerCapacity'));
			$('#centerAddress').html($(this).attr('centerAddress'));
	...
	$("form#addExaminationCenter").submit(function(e) { 
    e.preventDefault()
    var formData  = new FormData(this);

      $.ajax({
                url: base_url+"ajaxcall/add_examinationCenter",
                type: 'POST',
                data: formData,
                 beforeSend: function () {
                $('.edu_preloader').css('background-color','rgba(255,255,255,0.80)').css('display','block');;
                ...
                success: function(resp){
									...
									if(resp['status'] == '1'){
										...
										dataTableObj.ajax.url(base_url+targetTableUrl).load();
										$("#center_id").val('');
										// $('#input_feilds_teacher').find('[name="attachment"]').removeClass('error');
									}else if(resp['status'] == '2'){
										toastr.error(resp['msg']);
										$('.edu_preloader').fadeOut();
										return false;
	...
	$("form#addNewScholarship").submit(function(e) { 
    e.preventDefault()
    var formData  = new FormData(this);
    // var id = $("#bookId").val();
    // formData.append('book_id',id);
      ...
      $.ajax({
                url: base_url+"ajaxcall/add_scholarshp",
                type: 'POST',
                data: formData,
                 beforeSend: function () {
                $('.edu_preloader').css('background-color','rgba(255,255,255,0.80)').css('display','block');;
	...
	$("form#generateAdmitCard2022a").submit(function(e) { 
    e.preventDefault()
    var formData  = new FormData(this);
      $.ajax({
                url: base_url+"ajaxcall/generateAdmitCard",
                type: 'POST',
                data: formData,
                 beforeSend: function () {
                $('.edu_preloader').css('background-color','rgba(255,255,255,0.80)').css('display','block');;
	...
	$("form#addNewBook").submit(function(e) { 
            e.preventDefault()
            var formData  = new FormData(this);
            // var id = $("#bookId").val();
            // formData.append('book_id',id);
              ...
              $.ajax({
                        url: base_url+"ajaxcall/add_book",
                        type: 'POST',
                        data: formData,
                         beforeSend: function () {
                        $('.edu_preloader').css('background-color','rgba(255,255,255,0.80)').css('display','block');;
...
$("form#sellBookOrder").submit(function(e) { 
  e.preventDefault()
  var formData  = new FormData(this);

    $.ajax({
        url: base_url+"ajaxcall/sellBookOrder",
        type: 'POST',
        data: formData,
         beforeSend: function () {
        $('.edu_preloader').css('background-color','rgba(255,255,255,0.80)').css('display','block');;
	...
	$(document).on('click','.addNewBooks',function(){
		var validchk = validate_form($(this).closest('form'));
		if(validchk == 'valid'){
			var formdata = new FormData($(this).closest('form')[0]);
			var id = $("#bookId").val();
	...
	$(document).on('change','.changeNegativeType', function(){
		if($(this).val() == '1'){
			$('.negativehideshow').removeClass('hide');
			$('.negativehideshow .negativeClass').addClass('require');
			// $('.negativehideshow .chooseTime').addClass('require');
		}else{
			$('.negativehideshow').addClass('hide');
			$('.negativehideshow .negativeClass').removeClass('require');
			// $('.negativehideshow .chooseTime').removeClass('require');
		}
	});
	...
	$(document).on('click','.addEditHomewrk',function(){
		...
		$(document).on('click','.addEditNotes',function(){
		var validchk = validate_form($(this).closest('.form'));
		var id = $(this).attr('data-id');
		// alert($("#batch_id").val());
		if(validchk == 'valid'){
...
// 			}
	...
	$(document).on('click','.isDeletedData', function(){
        ...
        .then((willDelete) => {
          if (willDelete) {
			...
			$.ajax({
				method: "POST",
				url: base_url+"ajaxcall/isDeletedData",
				data: {'id':$(this).attr('data-id'),'table':$(this).attr('data-table'),'file':file},
				success: function(resp){
					var resp = $.parseJSON(resp);
					if(resp['status'] == '1'){
    ...
    $('.search_teachers').select2({
      ajax: {
    	...
    	$(document).on('click','.edit_video_url', function(){
			...
			$('#add_video_popup #id').val($(this).attr('data-id'));

			$('#add_video_popup #title').val($(this).attr('data-title'));

			 let batch_name=$(this).attr('data-batch');
			...
			let val=[];
			 for(var i=0;i<batch_data.length;i++){
			 	...
			 	val.push($('#add_video_popup #batch').find("option:contains('"+$.trim(batch_data[i])+"')").val());
			 }

			 $('#add_video_popup #batch').val(val).trigger('change.select2');
			...
			$('#add_video_popup [name="subject"]').val(val_sub).trigger('change.select2');
			 ...
			 parentDiv.find('input[name="video_type"][value="' + $(this).attr('data-type') + '"]').prop('checked', true);
